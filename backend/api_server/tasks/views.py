...
class SingleTaskView(APIView):
    ...
    def get(self, request, taskId=None, format=None):
        try:
            ...
            return JsonResponse({'error': "No valid Task found"}, status=500)
        except Exception as e:
            print("Error - ", str(e))
            return JsonResponse({'error': str(e)}, status=500)
        
    def patch(self, request, taskId=None, format=None):
        try:
    ...
    def delete(self, request, taskId=None, format=None):
        try:
            task_obj = Task.objects.get(pk=taskId)
